...
module ``about lists`` =
    ...
    let BuildingNewLists() =
        ...
        // so you _can_, but not in default implementation
        // and not properties -- cannot assign to first.Head anyway
        let mutable mutFirst = first
        mutFirst <- "apple" :: mutFirst
        AssertEquality mutFirst ["apple"; "grape"; "peach"]
    ...
    // so it has to copy the whole list
    // a -> b -> c -> unit
    // c -/-> d
    // a' -> b' -> c' -> d -> unit
    // appending is bad then
    ...
    let CreatingListsWithComprehensions() =
        ...
        let list = [for i in 0..3 do yield i*i ]
        AssertEquality list [0; 1; 4; 9]
